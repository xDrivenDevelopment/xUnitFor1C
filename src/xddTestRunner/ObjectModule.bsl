...
Процедура Инициализация()
	...
	Если ЭтоВстроеннаяОбработка() Тогда
		...
		// может быть использван префикс, который определяем имена объектов
	...
	Иначе
		ИспользуемыйПрефиксПодсистемы = ""; // Если это внешняя обработка, то префикс не используется
	КонецЕсли;
...
Функция Плагин(Знач Идентификатор) Экспорт
	...
	ИдентификаторПлагинаСПрефиксомПодсистемы = ДополнитьИдентификаторПрефиксомПодсистемы(Идентификатор, ИспользуемыйПрефиксПодсистемы);
	ИдентификаторПлагинаБезПрефиксаПодсистемы = ПолучитьИдентификаторБезПрефиксаПодсистемы(Идентификатор, ИспользуемыйПрефиксПодсистемы);
	Плагин = Плагины[ИдентификаторПлагинаБезПрефиксаПодсистемы];
	Если ТипЗнч(Плагин) = Тип("Строка") Тогда
		// Если подсистема тестирования встроена в конфигурацию, 
		// то плагины получаем также из встроенных в конфигурацию обработок
		Если ЭтоВстроеннаяОбработка() Тогда
			Плагин = ПолучитьВстроеннуюОбработку(ИдентификаторПлагинаСПрефиксомПодсистемы, ИспользуемыйПрефиксПодсистемы);
			// Заполняем служебные поля, если они имеются
			Если ПеременнаяСодержитСвойство(Плагин, "ИспользуемыйПрефиксПодсистемы") Тогда
				Плагин.ИспользуемыйПрефиксПодсистемы = ИспользуемыйПрефиксПодсистемы; 	
			КонецЕсли;
			Если ПеременнаяСодержитСвойство(Плагин, "ПодсистемаВстроенаВКонфигурацию") Тогда
				Плагин.ПодсистемаВстроенаВКонфигурацию = ЭтоВстроеннаяОбработка(); 	
			КонецЕсли; 			
		Иначе
	...
	Для каждого ДочернийУзел Из СтрокиКонтейнера Цикл
		...
		ИначеЕсли ДочернийУзел.Тип = ЭтотОбъект.ТипыУзловДереваТестов.Элемент Тогда
			Если УзелДереваТестовУдовлетворяетФильтру(ДочернийУзел, ФильтрДляДочернихУзлов) Тогда
				...
				// Заполняем служебные поля, если они имеются
				Если ПеременнаяСодержитСвойство(Контекст, "ИспользуемыйПрефиксПодсистемы") Тогда
					Контекст.ИспользуемыйПрефиксПодсистемы = ИспользуемыйПрефиксПодсистемы; 	
				КонецЕсли;
				Если ПеременнаяСодержитСвойство(Контекст, "ПутьКФайлуПолный") Тогда
					Контекст.ПутьКФайлуПолный = ДочернийУзел.Путь; 	
				КонецЕсли;
				Если ПеременнаяСодержитСвойство(Контекст, "ПодсистемаВстроенаВКонфигурацию") Тогда
					Контекст.ПодсистемаВстроенаВКонфигурацию = ЭтоВстроеннаяОбработка(); 	
				КонецЕсли;
...
Функция ПолучитьВстроеннуюОбработку(Идентификатор, ИспользуемыйПрефиксПодсистемы)
	...
	Если НЕ Метаданные.Обработки.Найти(ИдентификаторСПрефиксомПодсистемы) = Неопределено Тогда
		Возврат Обработки[ИдентификаторСПрефиксомПодсистемы].Создать();
	ИначеЕсли НЕ Метаданные.Обработки.Найти(ИдентификаторБезПрефиксаПодсистемы) = Неопределено Тогда
		Возврат Обработки[ИдентификаторБезПрефиксаПодсистемы].Создать();
	Иначе
		ВызватьИсключение "Не найдена встроенная в конфигурацию обработка """+Идентификатор+"""";
	КонецЕсли;
