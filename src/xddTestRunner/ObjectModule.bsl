	...
	Если ТипЗнч(Плагин) = Тип("Строка") Тогда
		...
		Если ЭтотОбъект.ЭтоВстроеннаяОбработка Тогда
			...
			Если ПеременнаяСодержитСвойство(Плагин, "ПодсистемаВстроенаВКонфигурацию") Тогда
				Плагин.ПодсистемаВстроенаВКонфигурацию = ЭтотОбъект.ЭтоВстроеннаяОбработка; 	
			КонецЕсли; 			
		Иначе
	...
	Для каждого ДочернийУзел Из СтрокиКонтейнера Цикл
		...
		ИначеЕсли ДочернийУзел.Тип = ЭтотОбъект.ТипыУзловДереваТестов.Элемент Тогда
			Если УзелДереваТестовУдовлетворяетФильтру(ДочернийУзел, ФильтрДляДочернихУзлов) Тогда
				...
				////ЗаполнитьСвойствоПриНаличии(Контекст, "ПодсистемаВстроенаВКонфигурацию", ИспользуемыйПрефиксПодсистемы);
				////
				...
				////	Контекст.ПодсистемаВстроенаВКонфигурацию = ЭтотОбъект.ЭтоВстроеннаяОбработка 	
				////КонецЕсли;
...
Функция ПолучитьВстроеннуюОбработку(Идентификатор, ИспользуемыйПрефиксПодсистемы) Экспорт
