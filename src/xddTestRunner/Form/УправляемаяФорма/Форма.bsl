...
Процедура Подключаемый_ИнтерактивныйВызовЗагрузчика(Команда)
	...
	Плагин(ИдентификаторВыбранногоЗагрузчика).ВыбратьПутьИнтерактивно(ЭтаФорма,ИдентификаторВыбранногоЗагрузчика);
КонецПроцедуры 
...
Процедура ПодключитьВнешнююОбработку(ФайлОбработки) Экспорт
	...
	Если ПодключенныеВнешниеОбработки.Получить(ФайлОбработки.ИмяБезРасширения) = Неопределено Тогда
		ЭтоОтчет = (НРег(ФайлОбработки.Расширение) = ".erf");
		Оп = Новый ОписаниеОповещения("ПодключитьВнешнююОбработкуЗавершение", ЭтаФорма,Новый Структура("АдресХранилища, ФайлОбработки, ЭтоОтчет", АдресХранилища, ФайлОбработки, ЭтоОтчет));
