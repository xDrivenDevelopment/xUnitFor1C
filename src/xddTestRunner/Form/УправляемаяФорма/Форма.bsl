...
Функция ПолучитьДополнительнуюОбработкуПлагина(ИмяПлагина)
	Возврат Справочники.ДополнительныеОтчетыИОбработки.НайтиПоНаименованию(ИмяПлагина);
КонецФункции
...
Функция Плагин(Знач Идентификатор) Экспорт
	...
	ИначеЕсли ОбъектПлагин = Истина Тогда
		...
		ДополнительнаяОбработкаПлагина= ПолучитьДополнительнуюОбработкуПлагина(ИмяПлагина);
		
		Если Не ДополнительнаяОбработкаПлагина.Пустая() Тогда
			
			ИмяПодключенногоПлагина= ДополнительныеОтчетыИОбработкиВызовСервера.ПодключитьВнешнююОбработку(ДополнительнаяОбработкаПлагина);
	
			ОбъектПлагин = ПолучитьФорму("ВнешняяОбработка." + ИмяПодключенногоПлагина + ".Форма", , ЭтаФорма);
			...
			Попытка
				...
				Исключение
					...
					Если Инфо.Описание = "Метод объекта не обнаружен (Инициализация)" И
						...
						СтрНайти(Инфо.ИмяМодуля, "Форма.УправляемаяФорма.Форма") > 0
						Тогда
						Иначе
							ВызватьИсключение;
						КонецЕсли;
