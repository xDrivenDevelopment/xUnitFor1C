...
Функция ПодключитьПлагиныНаСервере(Знач ЗагружаемыеФайлы)
	...
	Для Каждого ОписаниеФайла Из ЗагружаемыеФайлы Цикл
		...
		Попытка
            ...
            Иначе
                ОбъектОбработки= Вычислить("ДополнительныеОтчетыИОбработкиВызовСервера.ОбъектВнешнейОбработки(ДополнительнаяОбработка)");
            КонецЕсли;
	...
	Для Каждого ОбъектОбработки Из ОбъектыПлагинов Цикл
		Попытка
			ОбъектОбработки.Инициализация(ЭтотОбъектНаСервере());
		Исключение
			Ошибка = "Возникла ошибка при инициализации плагина: " + ОбъектОбработки.Метаданные().Имя + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Сообщить(Ошибка);
		КонецПопытки;
	КонецЦикла;
