		...
		Попытка
			...
			Исключение
				...
				Если Инфо.Описание = "Метод объекта не обнаружен (Инициализация)" И
					СтрНайти(Инфо.ИсходнаяСтрока, "ОбъектПлагин.Инициализация(ЭтаФорма);") > 0 И
					СтрНайти(Инфо.ИмяМодуля, "xddTestRunner.Форма.УправляемаяФорма.Форма") > 0
					Тогда
					Иначе
						ВызватьИсключение;
					КонецЕсли;
...
Процедура ВыполнитьПакетныйЗапускСинхронно(Знач ПараметрыЗапуска)
	...
	Если ПараметрыЗапуска.Параметры_xddRun <> Неопределено Тогда
		РезультатыТестирования = ЗагрузитьИВыполнитьТесты_ПакетныйРежим(ПараметрЗапуска.Параметры_xddRun);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РезультатыТестирования) И ПараметрЗапуска.Параметры_xddReport <> Неопределено Тогда
...
Процедура ВыполнитьПакетныйЗапускАсинхронно(Знач ПараметрыЗапуска)
	...
	ИначеЕсли ПараметрЗапуска.xddShutdown = Истина Тогда
		Состояние.СледующийШаг = "ЗавершитьРаботу";
	Иначе
		ВызватьИсключение НСтр("ru = 'xUnit: Я не знаю что мне делать. Нет приказа на выполнение тестов или закрытие.'");
	КонецЕсли;
...
Процедура ВыполнитьШагПакетногоЗапускаИсключение(Знач ИнформацияОбОшибке, СтандартнаяОбработка, Знач Состояние) Экспорт
	...
	ТекстОшибки = "Ошибка при асинхронном режиме пакетного запуска:
	|Состояние: 
	|	Текущий шаг:  %1
	|	Следующий шаг:%2
	|	Исключение:   %3";
	
	// для унылых ретроградов, применяющих < 8.3.6 - придется без СтрШаблон
	ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", Состояние.ТекущийШаг);
	ТекстОшибки = СтрЗаменить(ТекстОшибки, "%2", Состояние.СледующийШаг);
	ТекстОшибки = СтрЗаменить(ТекстОшибки, "%3", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция СтрокаЭкспортируемыхОтчетов()
	Возврат Новый Структура("Отчет,Путь,ГенераторОтчета");
КонецФункции
...
Процедура СформироватьФайлКодаВозврата(Знач Параметры_xddExitCodePath, Знач РезультатыТестирования)
	Попытка
		...
		ГенераторКодаВозврата = ЭтаФорма.Плагин(ИдентификаторПлагина);
...
&НаКлиенте
Процедура ОткрытьИнструмент(Знач ИмяИнструмента, Знач ПутьИнструмента = "", Знач ИмяФормыИнструмента = "Форма")
	Перем АдресХранилища;
	...
	Если Объект.ЭтоВстроеннаяОбработка Тогда
		...
		ФормаИнструмента = ПолучитьФорму("Обработка." + ИмяИнструмента + "." + ИмяФормыИнструмента);
	...
	Иначе
		...
		ФайлИнструмента.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ОткрытьИнструментЗавершение1", ЭтаФорма, Новый Структура("АдресХранилища, ИмяИнструмента, ИмяФормыИнструмента, ФайлИнструмента", АдресХранилища, ИмяИнструмента, ИмяФормыИнструмента, ФайлИнструмента)));
		                                       
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьИнструментЗавершение1(Существует, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ОткрытьИнструментЗавершение", ЭтаФорма, Новый Структура("ИмяИнструмента, ИмяФормыИнструмента", ИмяИнструмента, ИмяФормыИнструмента));
