...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВстроенаВКонфигурацию = ОбъектНаСервере.ЭтоВстроеннаяОбработка();
	ИспользуемыйПрефиксПодсистемы = ОбъектНаСервере.ИспользуемыйПрефиксПодсистемы;
	
	ЭтаФорма.Заголовок = ОбъектНаСервере.ЗаголовокФормы();
	
	Если НЕ ВстроенаВКонфигурацию Тогда // Используемое имя файла меняется в зависимости от того
...
Процедура КомандаПерезагрузитьФорму(Команда)
	...
	Если ВстроенаВКонфигурацию Тогда // Для встроенной в конф. обработки открываем форму
...
Функция Плагин(Знач Идентификатор) Экспорт
	...
	ИдентификаторБезПрефиксовПодсистемы = ПолучитьИдентификаторБезПрефиксаПодсистемы(Идентификатор, ИспользуемыйПрефиксПодсистемы); 
	...
	ИначеЕсли ОбъектПлагин = Истина Тогда
		...
		ИмяПлагина = Объект.Плагины[ПолучитьИдентификаторБезПрефиксаПодсистемы(Идентификатор, ИспользуемыйПрефиксПодсистемы)];
		Попытка
			Если ВстроенаВКонфигурацию Тогда
				ОбъектПлагин = ПолучитьФорму("Обработка." + ДополнитьИдентификаторПрефиксомПодсистемы(ИмяПлагина, ИспользуемыйПрефиксПодсистемы) + ".Форма", , ЭтаФорма);	
				// Заполняем служебные поля, если они имеются
				Если ПеременнаяСодержитСвойство(ОбъектПлагин, "Объект") Тогда
					Если ПеременнаяСодержитСвойство(ОбъектПлагин.Объект, "ИспользуемыйПрефиксПодсистемы") Тогда
						ОбъектПлагин.Объект.ИспользуемыйПрефиксПодсистемы = ИспользуемыйПрефиксПодсистемы; 	
					КонецЕсли;
				КонецЕсли;
			Иначе
		...
		ИначеЕсли ДочернийУзел.Тип = Объект.ТипыУзловДереваТестов.Элемент Тогда
			Если УзелДереваТестовУдовлетворяетФильтру(ДочернийУзел, ФильтрДляДочернихУзлов) Тогда
				...
				Если ПеременнаяСодержитСвойство(КонтекстВыполнения, "ИспользуемыйПрефиксПодсистемы") Тогда
					КонтекстВыполнения.ИспользуемыйПрефиксПодсистемы = ИспользуемыйПрефиксПодсистемы; 	
				КонецЕсли;
				Если ПеременнаяСодержитСвойство(КонтекстВыполнения, "ПодсистемаВстроенаВКонфигурацию") Тогда
					КонтекстВыполнения.ПодсистемаВстроенаВКонфигурацию = ВстроенаВКонфигурацию; 	
				КонецЕсли; 	
...
Функция ВыполнитьТестовыйМетодНаСервере(Знач ОписаниеКонтекста, Знач ЭлементДереваТестов)
	...
	Если ОписаниеКонтекста.ЭтоВнутренняяОбработка Тогда
		КонтекстВыполнения = ПолучитьВстроеннуюОбработку(ОписаниеКонтекста.Идентификатор, ИспользуемыйПрефиксПодсистемы);
	Иначе	
	...
	Если ПеременнаяСодержитСвойство(КонтекстВыполнения, "ИспользуемыйПрефиксПодсистемы") Тогда
		КонтекстВыполнения.ИспользуемыйПрефиксПодсистемы = ИспользуемыйПрефиксПодсистемы; 	
	КонецЕсли;
	Если ПеременнаяСодержитСвойство(КонтекстВыполнения, "ПутьКФайлуПолный") Тогда
		КонтекстВыполнения.ПутьКФайлуПолный = ЭлементДереваТестов.Путь; 	
	КонецЕсли;
	Если ПеременнаяСодержитСвойство(КонтекстВыполнения, "ПодсистемаВстроенаВКонфигурацию") Тогда
		КонтекстВыполнения.ПодсистемаВстроенаВКонфигурацию = ВстроенаВКонфигурацию; 	
	КонецЕсли;
