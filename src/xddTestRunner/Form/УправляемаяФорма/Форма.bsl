...
&НаКлиенте
Перем ЗапущенныеТестКлиенты Экспорт;
...
Процедура ВыполнитьПакетныйЗапуск(Знач ПараметрЗапуска)
	...
	Если ПараметрыЗапуска.Свойство(ВозможныеКлючи.xddTestClient, Параметры_xddTestClient) Тогда
		ПодключитьТестКлиент_ПакетныйРежим(Параметры_xddTestClient);
	КонецЕсли;
...
&НаКлиенте
Процедура ПодключитьТестКлиент_ПакетныйРежим(Параметры_xddTestClient)
	...
	Для Каждого ОчередныеПараметры Из Параметры_xddTestClient Цикл
		Попытка
			...
			КонецЕсли;
			ЗапущенныеТестКлиенты.Добавить(Новый Структура("ТестКлиент,Порт", ТестКлиент, ПользовательПарольПорт[2]));
		Исключение
			Инфо = ИнформацияОбОшибке();
...
&НаКлиенте
Функция ПодключитьТестКлиент(ИмяПользователя = "", Пароль = "", Порт = 1538)
...
&НаКлиенте
Процедура ЗавершитьВсеТестКлиенты()
