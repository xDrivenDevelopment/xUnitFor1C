...
Процедура КомандаПерезагрузитьФорму(Команда)
	...
	Иначе
		...
		ВыбранноеИмяФайла = "";
		Если ПоместитьФайл(АдресХранилища, ВыбранноеИмяФайла, ВыбранноеИмяФайла, Ложь, ЭтаФорма.УникальныйИдентификатор) Тогда
			КомандаПерезагрузитьФормуЗавершение(Истина, АдресХранилища, ВыбранноеИмяФайла, Неопределено);
		КонецЕсли;
		// Back to 8.2
...
Процедура ЗагрузитьНастройкиИзФайла(Команда)
	...
	Если ДиалогВыбора.Выбрать() Тогда
		...
		ВыбранныеФайлы.Добавить(ДиалогВыбора.ПолноеИмяФайла);
		ЗагрузитьНастройкиИзФайлаЗавершение(ВыбранныеФайлы, Неопределено);
	КонецЕсли;
	/// Back to 8.2
...
Процедура СформироватьОтчетОТестированииИЭкспортировать_ПакетныйРежим(Знач Параметры_xddReport, Знач РезультатыТестирования)
	...
	Для Каждого ОчередныеПараметры Из Параметры_xddReport Цикл
		Попытка
			...
			ОписаниеОшибки = Плагин("СтроковеУтилиты").СтрШаблон82("УФ ПутьКОтчету %1", ПутьКОтчету);
			ЗафиксироватьОшибкуВЖурналеРегистрации("xUnitFor1C." + ИдентификаторГенератораОтчета, ОписаниеОшибки, Истина);
...
&НаСервере
Процедура ЗафиксироватьОшибкуВЖурналеРегистрации(Знач ИдентификаторГенератораОтчета, Знач ОписаниеОшибки, ЭтоИнформация = Ложь)
	Уровень = ?(ЭтоИнформация, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка); 
	ЗаписьЖурналаРегистрации(ИдентификаторГенератораОтчета, Уровень, , , ОписаниеОшибки);
КонецПроцедуры
// } Пакетный запуск
...
Процедура ОткрытьИнструмент(Знач ИмяИнструмента, Знач ПутьИнструмента = "", Знач ИмяФормы = "Форма")
	...
	Иначе
		...
		ВыбранноеИмяФайла = "";
		Если ПоместитьФайл(АдресХранилища, ФайлИнструмента.ПолноеИмя, ВыбранноеИмяФайла , Ложь, ЭтаФорма.УникальныйИдентификатор) Тогда
			ДопПараметры = Новый Структура("ИмяИнструмента, ИмяФормыИнструмента", ИмяИнструмента, ИмяФормы);
			ОткрытьИнструментЗавершение(Истина, АдресХранилища, ВыбранноеИмяФайла, ДопПараметры);
		КонецЕсли;
		/// Back to 8.2
...
Процедура ПодключитьВнешнююОбработку(ФайлОбработки) Экспорт
	...
	Если ПодключенныеВнешниеОбработки.Получить(ФайлОбработки.ИмяБезРасширения) = Неопределено Тогда
		...
		ВыбранноеИмяФайла = "";
		Если ПоместитьФайл(АдресХранилища, ФайлОбработки.ПолноеИмя , ВыбранноеИмяФайла, Ложь, ЭтаФорма.УникальныйИдентификатор) Тогда
			ДопПараметры = Новый Структура("ФайлОбработки, ЭтоОтчет", ФайлОбработки, ЭтоОтчет);	
			ПодключитьВнешнююОбработкуЗавершение(Истина, АдресХранилища, ВыбранноеИмяФайла, ДопПараметры);
		КонецЕсли;
		/// Back to 8.2
	КонецЕсли;
КонецПроцедуры
