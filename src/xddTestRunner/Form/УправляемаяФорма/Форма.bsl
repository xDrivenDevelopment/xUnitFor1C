...
Процедура ПодключитьФайлОбработкиНаСервере(Знач АдресХранилища, Знач ИдентификаторОбработки, ЭтоОтчет = Ложь)
	ДополнительнаяОбработка= ПолучитьДополнительнуюОбработкуБСП(ИдентификаторОбработки);
	Если ДополнительнаяОбработка = Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		Выполнить("ДополнительныеОтчетыИОбработкиВызовСервера.ПодключитьВнешнююОбработку(ДополнительнаяОбработка)");
	КонецЕсли;
КонецПроцедуры
