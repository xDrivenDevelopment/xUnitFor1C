...
Процедура ВыполнитьПакетныйЗапуск(Знач ПараметрЗапуска)
	...
	Если ПараметрыЗапуска.Свойство(ПарсерКоманднойСтроки.ВозможныеКлючи.xddExitCodePath, Параметры_xddExitCodePath) Тогда
		СформироватьФайлКодаВозврата(Параметры_xddExitCodePath[0], РезультатыТестирования);
	КонецЕсли;
...
Процедура СформироватьФайлКодаВозврата(Знач ПутьФайлаКодаВозврата, Знач РезультатыТестирования)
	Попытка
		КодВозврата = 1;
		Если Не ЗначениеЗаполнено(РезультатыТестирования) ИЛИ РезультатыТестирования.Состояние = СостоянияТестов.Сломан Тогда
			КодВозврата = 0;
		КонецЕсли;
		
		Сообщение = СтрШаблон("КодВозврата <%1>", КодВозврата);
		ЗаписьЖурналаРегистрации("xUnitFor1C", УровеньЖурналаРегистрации.Информация, , , Сообщение);
