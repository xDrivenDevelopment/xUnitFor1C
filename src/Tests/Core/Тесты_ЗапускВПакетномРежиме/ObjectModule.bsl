...
Процедура ТестДолжен_ПроверитьЗапускВПакетномРежиме_ТолстыйКлиент_ДваОтчетаТестирования() Экспорт
	...
	ФайлСОтчетомОТестировании2Существует = СуществуетФайлОтчетаAllure(
		ФайлСОтчетомОТестировании2.Путь, 
		ФайлСОтчетомОТестировании2.ИмяБезРасширения
	);
	Ожидаем.Что(ФайлСОтчетомОТестировании2Существует, "ФайлСОтчетомОТестировании2 не существует").ЭтоИстина();
	
	Ожидаем.Что(ФайлКодаВозврата.Существует(), "ФайлКодаВозврата не существует").ЭтоИстина();
КонецПроцедуры

Функция СуществуетФайлОтчетаAllure(Путь, ИмяБезРасширения)
	
	ШаблонИмениФайла = "*-" + ИмяБезРасширения + "-testsuite.xml";
	
	НайденныеФайлы = НайтиФайлы(Путь, ШаблонИмениФайла, Ложь);
...
Процедура ПроверитьПравильностьПакетногоЗапуска(Знач ОжидаемоеКоличествоЗеленыхТестов, Знач ОжидаемоеКоличествоУпавшихТестов, Знач ПроверятьЛогUI = Истина)
	...
	Если КоличествоУпавшихТестов <> ОжидаемоеКоличествоУпавшихТестов или КоличествоЗеленыхТестов <> ОжидаемоеКоличествоЗеленыхТестов Тогда
		Для каждого КлючЗначение Из РезультатыТестирования.УпавшиеТесты Цикл
			...
			Сообщить(СтроковыеУтилиты.СтрШаблон82("Упал тест <%1>, ошибка %2%3", УпавшийТест.Имя, Символы.ПС, УпавшийТест.ТекстОшибки));
		КонецЦикла;
	КонецЕсли;
