...
Функция УстановитьОтборВСписке(ПараметрыОтбора) Экспорт
	...
	Для Каждого Элемент Из МассивОтбора Цикл
		...
		// ЗаголовокОкнаДанных, которое откроется после выбора типа 
		...
		Иначе
			...
			Если НЕ ТипДанныхВыбран Тогда
				...
				Возврат Ложь;
			КонецЕсли;
			ВыборДанных= ПолучитьСтруктуруОкнаПриложения(ТестКлиент, Элемент.ЗаголовокОкнаДанных);
...
Функция СформироватьСтруктуруПоиска()
	...
	Для каждого Элемент Из Отладка_СтруктураПоиска Цикл
		ЭлементПоиска= Новый Структура("Ключ,Значение,ПредставлениеТипаДанных,ЗаголовокОкнаДанных");
