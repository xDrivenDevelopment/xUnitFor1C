...
&НаКлиенте
Процедура ОткрытьФормуСписка(ПолноеИмяОбъектаМетаданных) Экспорт
	
	//+debug
	
	Сообщить("ОткрытьФормуСписка " + ПолноеИмяОбъектаМетаданных);
	
	Сообщить("ЗапущенныеТестКлиенты.Количество= " + ЗапущенныеТестКлиенты.Количество());
	
	ОсновноеОкно= ПолучитьОсновноеОкно();
	
	Сообщить("Основное окно " + ТипЗнч(ОсновноеОкно));
...
Функция ПолучитьОсновноеОкно() Экспорт
	...
	Для каждого ТестируемоеОкно Из ТестируемыеОкна Цикл
		Если ТестируемоеОкно.Основное Тогда
			ОсновноеОкно= ТестируемоеОкно;
			Прервать;
		КонецЕсли;
	КонецЦикла; 
...
Функция ПолучитьОкно(ТекстЗаголовка) Экспорт
	...
	Возврат ТестКлиент.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"), ТекстЗаголовка, , 20);	
...
Процедура ОткрытьСписок(Команда)
	ОткрытьФормуСписка("Документ.Документ1");
КонецПроцедуры
