...
Функция НайтиВСписке(СтруктураПоиска) Экспорт
	...
	Возврат ТестируемыйЭлемент.ПерейтиКСтроке(ПреобразоватьКОписаниюСтроки(СтруктураПоиска));	
...
&НаКлиенте
Процедура УстановитьОтборВСпискеПоРегистратору(ЗаголовокСпискаРегистратора, ПредставлениеТипаРегистратора, СтруктураПоискаРегистратора) Экспорт
	...
	//ЕстьРегистратор= ТаблицаВыбраныеПоля.ПерейтиКСтроке(ПреобразоватьКОписаниюСтроки("Доступные поля=Регистратор"), НаправлениеПереходаКСтроке.Вниз);
	
	// Добавляем в отбор регистратор когда его там нет
	
	Если Не ЕстьРегистратор Тогда
		ТаблицаДоступныеПоля.ПерейтиКСтроке(ПреобразоватьКОписаниюСтроки("Доступные поля=Регистратор"));
	...
	ТаблицаВыбраныеПоля.ПерейтиКСтроке(ПреобразоватьКОписаниюСтроки("Представление=Регистратор"));
	...
	ТаблицаТипов.ПерейтиКСтроке(ПреобразоватьКОписаниюСтроки("=" + ПредставлениеТипаРегистратора));
	...
	// Из отладки работает
	//ОкноПриложенияВыборРегистратора = ТестКлиент.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"), ЗаголовокСпискаРегистратора, , 30);
	...
	ТаблицаДокументов.ПерейтиКСтроке(ПреобразоватьКОписаниюСтроки(СтруктураПоискаРегистратора));
...
// 		Строка			"Имя=Значение;"
// 		Соответствие
Функция ПреобразоватьКОписаниюСтроки(Параметр) 
	
	ТипПараметра= ТипЗнч(Параметр);
	
	ОписаниеСтроки= Неопределено;
	
	Если ТипПараметра = Тип("Соответствие") Тогда
