...
<<<<<<< HEAD
	Если ЭтоВстроеннаяОбработка() Тогда
		ПостроительДереваТестов = Обработки.ПостроительДереваТестов.Создать();
		ЗагрузчикФайла = Обработки.ЗагрузчикФайла.Создать();
	Иначе
...
Функция ЭтоВстроеннаяОбработка() Экспорт
	...
	Возврат ОбъектНаСервере.ЭтоВстроеннаяОбработка();
