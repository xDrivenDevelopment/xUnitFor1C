...
// Процедура - Проверить загрузку макета
//
// Параметры:
//  ТабличныйДокумент	 - ТабличныйДокумент	 - табличный документ с данными для загрузки
//  ПроверкаЗагрузки	 - Булево				 - признак того, что проводится только проверка загрузки
//                                                 и данные будут удалены по окончании проверки.
//
Процедура ПроверитьЗагрузкуМакета(ТабличныйДокумент, ПроверкаЗагрузки = Истина) Экспорт
	НачатьТранзакцию();
	...
	Иначе
		Сообщить("Загрузка объектов в ИБ из макета данных выполнена успешно.");
	КонецЕсли;
	...
	Если ПроверкаЗагрузки И ТранзакцияАктивна() Тогда
		ОтменитьТранзакцию();
	КонецЕсли;
КонецПроцедуры
