...
Функция СоздатьОтчет(КонтекстЯдраПараметр, РезультатыТестирования) Экспорт
	УстановитьКонтекстЯдра(КонтекстЯдраПараметр);
	...
	ЭтотОбъект.СостоянияТестов = КонтекстЯдраПараметр.СостоянияТестов;
	ЭтотОбъект.СтроковыеУтилиты = КонтекстЯдра.Плагин("СтроковыеУтилиты");
...
Функция ПолучитьУникальноеИмяФайла(Знач ИмяФайла)
	...
	ИмяФайла = СтроковыеУтилиты.СтрШаблон82("%1-%2-testsuite.xml", ГУИД, Файл.ИмяБезРасширения);
	ИмяФайла = СтроковыеУтилиты.СтрШаблон82("%1/%2", Файл.Путь, ИмяФайла); 
	Возврат ИмяФайла;
КонецФункции
...
Процедура ПроверитьИмяФайлаРезультатаAllure(ИмяФайла) Экспорт
	...
	Если Найти(Файл.Имя, "-testsuite") = 0 Тогда
		Сообщение = СтроковыеУтилиты.СтрШаблон82("%1
