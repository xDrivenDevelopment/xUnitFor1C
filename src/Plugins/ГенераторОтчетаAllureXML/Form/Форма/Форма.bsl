...
Функция ПолучитьУникальноеИмяФайла(Знач ИмяФайла)
	...
	ИмяФайла = ЭтотОбъектНаСервере().СтроковыеУтилиты.СтрШаблон82("%1-%2-testsuite.xml", ГУИД, Файл.ИмяБезРасширения);
	ИмяФайла = ЭтотОбъектНаСервере().СтроковыеУтилиты.СтрШаблон82("%1/%2", Файл.Путь, ИмяФайла); 
	Возврат ИмяФайла;
КонецФункции
