...
Функция СоздатьУтилиту(ИмяУтилиты)
	Если ЭтоВстроеннаяОбработка Тогда
		ИмяУтилитыСПрефиксомПодсистемы = ДополнитьИдентификаторПрефиксомПодсистемы(ИмяУтилиты, ИспользуемыйПрефиксПодсистемы); 
		Возврат Обработки[ИмяУтилитыСПрефиксомПодсистемы].Создать();
	Иначе
		Если ЗначениеЗаполнено(ПутьКФайлуПолный) Тогда
			ФайлОбработки = Новый Файл(ПутьКФайлуПолный);
		Иначе
			ФайлОбработки = Новый Файл(ЭтотОбъект["ИспользуемоеИмяФайла"]);
		КонецЕсли;
		КорневойКаталогФреймворка = ПолучитьКорневойКаталогФреймворка(ФайлОбработки.Путь);
		Возврат ВнешниеОбработки.Создать(КорневойКаталогФреймворка+"Plugins/"+ИмяУтилиты+".epf", Ложь);
	КонецЕсли;
КонецФункции
